...
template_res = importlib.resources.files(RESOURCE_PACKAGE).joinpath("config/config.yaml")
with importlib.resources.as_file(template_res) as config_file:
    with open(config_file, "r") as stream:
        template_res = importlib.resources.files(RESOURCE_PACKAGE).joinpath("locale")
        with importlib.resources.as_file(template_res) as locale:
            try:
